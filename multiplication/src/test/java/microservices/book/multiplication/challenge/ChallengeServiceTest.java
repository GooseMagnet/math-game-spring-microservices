...
//        challengeService = new ChallengeServiceImpl(userRepository, attemptRepository, gameClient);
    ...
    @Test
    void ShouldRetrieveLastAttemptsForUser() {
        // given
        ...
        given(attemptRepository.findTop10ByUserAliasOrderByIdDesc(anyString())).willReturn(expectedAttempts);

        // when
        List<ChallengeAttempt> actualAttempts = challengeService.getStatsForUser("john_doe");

        // then
        then(actualAttempts).isEqualTo(expectedAttempts);
    }
}
